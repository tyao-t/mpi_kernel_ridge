...
{
  if (rank == 0) {
    double *sums = (double *)malloc(sizeof(double) * (n_feature + 1) * nprocs);
    int i, j, *n = (int *)malloc(sizeof(int) * nprocs);
    MPI_Gather(sum, n_feature + 1, MPI_DOUBLE, sums, n_feature + 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Gather(&n_sample, 1, MPI_INT, n, 1, MPI_INT, 0, MPI_COMM_WORLD);

    for (i = 0; i <= n_feature; i++) avg[i] = 0;
    for (i = 0; i < nprocs; i++) {
      for (j = 0; j <= n_feature; j++)
        avg[j] += sums[i * (n_feature + 1) + j];
      if (i != 0) n[0] += n[i];
    }
    for (i = 0; i <= n_feature; i++) avg[i] /= n[0];

    free(sums); free(n);
  } else {
    MPI_Gather(sum, n_feature + 1, MPI_DOUBLE, NULL, n_feature + 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Gather(&n_sample, 1, MPI_INT, NULL, 1, MPI_INT, 0, MPI_COMM_WORLD);
  }
  MPI_Bcast(avg, n_feature + 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
}
