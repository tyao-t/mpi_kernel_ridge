...
- Achieved 43948.3 RMSE on the test set (random_state=17) and a RMSE between ~44000 + ~5000 in general (for other train-test splits).
...
Code compatible with (be supplied as a custom model into) scikit-learn's GridSearchCV and BayesSearchCV, given that `__init__`, `fit`, and `score` interface functions are implemented.